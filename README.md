...
Custom SVG map marker icons for google-map or the Google Maps Javascript API.
...
## Live Demo
See the [project demo page](https://mlisook.github.io/plastic-map-marker-svg) for demos and code examples.

## Versions
There is a version for Polymer 3.0 projects and a version for Polymer 2.x projects.

### Install for Polymer 2
```
bower install --save plastic-map-marker-svg#^0.0.3
```
### Install for Polymer 3
```
...
While it's not actual data binding, marker definitions can have substitution points (see samples above), marked with [[some.data.path]] that will be substituted in when you request the marker.
...
```
```JS
PlasticMapMarkerSet.getMarkerSetIcon('sample-markers', 'tdpin', 64, 64, myDataObject)
    .then((icon) => {
        this.icon = icon;
        this.points = [{
            lat: 37.779,
...
<plastic-aspect-ratio style="width:100%;" aspect-width="4" aspect-height="3">
    <google-map id="gm" style="width:100%;height:100%;" latitude="37.779" longitude="-122.3892" 
        min-zoom="9" max-zoom="11" language="en" api-key="[[apiKey]]">
        <template is="dom-repeat" items="[[points]]">
            <plastic-map-marker-svg slot="markers" latitude="[[item.lat]]" longitude="[[item.lng]]" 
                icon-name="[[item.markerStyle.iconName]]" icon-height="[[item.markerStyle.height]]" icon-width="[[item.markerStyle.width]]" 
                icon-data="[[item.markerStyle.iconData]]">
...
MIT
## Support
Support is available for both the Polymer 2 and Polymer 3 versions. Please submit issues or questions through the github repository.
## Contributions
